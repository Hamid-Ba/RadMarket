...
namespace RadMarket.Query.Queries
{
    public class ProductQuery : IProductQuery
    {
        ...
        public async Task<IEnumerable<ProductQueryVM>> GetAllBestSells(int take = 0)
        {
            ...
            var products = await _storeContext.Products
                ...
                }).AsNoTracking().OrderByDescending(p => p.OrderCount).ToListAsync();
        ...
        public async Task<IEnumerable<ProductQueryVM>> GetAllWhichHasDiscount(int take = 0)
        {
            var discounts = await _discountContext.Discounts.Where(d => d.StartDate <= DateTime.Now && d.EndDate >= DateTime.Now).
                Select(d => new
                {
